...
pub fn ccloudgaming_createnonce_request_unpack(buf []byte) ?CCloudGaming_CreateNonce_Request {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.platform = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			2 {
...
pub fn ccloudgaming_createnonce_response_unpack(buf []byte) ?CCloudGaming_CreateNonce_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.nonce = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			2 {
...
pub fn ccloudgaming_timeremaining_unpack(buf []byte) ?CCloudGaming_TimeRemaining {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.minutes_remaining = vproto.unpack_uint32_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
