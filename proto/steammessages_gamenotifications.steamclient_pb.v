...
pub fn cgamenotifications_localizedtext_unpack(buf []byte) ?CGameNotifications_LocalizedText {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			3 {
				i, res.rendered_text = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn cgamenotifications_userstatus_unpack(buf []byte) ?CGameNotifications_UserStatus {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.state = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			3 {
				i, res.title = zzz_vproto_internal_unpack_cgamenotifications_localizedtext(cur_buf,
					tag_wiretype.wire_type) ?
			}
			4 {
				i, res.message = zzz_vproto_internal_unpack_cgamenotifications_localizedtext(cur_buf,
					tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
...
pub fn cgamenotifications_createsession_request_unpack(buf []byte) ?CGameNotifications_CreateSession_Request {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.context = vproto.unpack_uint64_field(cur_buf, tag_wiretype.wire_type) ?
			}
			3 {
				i, res.title = zzz_vproto_internal_unpack_cgamenotifications_localizedtext(cur_buf,
					tag_wiretype.wire_type) ?
...
pub fn cgamenotifications_enumeratesessions_request_unpack(buf []byte) ?CGameNotifications_EnumerateSessions_Request {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			3 {
				i, res.include_all_user_messages = vproto.unpack_bool_field(cur_buf, tag_wiretype.wire_type) ?
			}
			4 {
				i, res.include_auth_user_message = vproto.unpack_bool_field(cur_buf, tag_wiretype.wire_type) ?
			}
			5 {
...
pub fn cgamenotifications_session_unpack(buf []byte) ?CGameNotifications_Session {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.appid = vproto.unpack_uint64_field(cur_buf, tag_wiretype.wire_type) ?
			}
			3 {
				i, res.context = vproto.unpack_uint64_field(cur_buf, tag_wiretype.wire_type) ?
			}
...
pub fn gamenotificationsettings_unpack(buf []byte) ?GameNotificationSettings {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.allow_notifications = vproto.unpack_bool_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn cgamenotifications_onuserstatuschanged_notification_unpack(buf []byte) ?CGameNotifications_OnUserStatusChanged_Notification {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			4 {
				i, res.status = zzz_vproto_internal_unpack_cgamenotifications_userstatus(cur_buf,
					tag_wiretype.wire_type) ?
			}
			5 {
				i, res.removed = vproto.unpack_bool_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
