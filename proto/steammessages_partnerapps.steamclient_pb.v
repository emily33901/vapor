...
pub fn cpartnerapps_requestuploadtoken_response_unpack(buf []byte) ?CPartnerApps_RequestUploadToken_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.upload_token = vproto.unpack_uint64_field(cur_buf, tag_wiretype.wire_type) ?
			}
			2 {
				i, res.location = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			3 {
				i, res.routing_id = vproto.unpack_uint64_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn (a CPartnerApps_RequestUploadToken_Response) eq(b CPartnerApps_RequestUploadToken_Response) bool {
	return true && a.upload_token == b.upload_token && a.location == b.location && a.routing_id ==
		b.routing_id
}
...
pub fn cpartnerapps_finishuploadkvsign_response_unpack(buf []byte) ?CPartnerApps_FinishUploadKVSign_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.signed_installscript = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn cpartnerapps_finishuploadlegacydrm_request_unpack(buf []byte) ?CPartnerApps_FinishUploadLegacyDRM_Request {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			5 {
				i, res.tool_name = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn (a CPartnerApps_FinishUploadLegacyDRM_Request) eq(b CPartnerApps_FinishUploadLegacyDRM_Request) bool {
	return true && a.upload_token == b.upload_token && a.routing_id == b.routing_id &&
		a.app_id == b.app_id && a.flags == b.flags && a.tool_name == b.tool_name
}
...
pub fn cpartnerapps_finishuploadlegacydrm_response_unpack(buf []byte) ?CPartnerApps_FinishUploadLegacyDRM_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.file_id = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn cpartnerapps_finishuploaddepot_request_unpack(buf []byte) ?CPartnerApps_FinishUploadDepot_Request {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			5 {
				i, res.build_flags = vproto.unpack_uint32_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn (a CPartnerApps_FinishUploadDepot_Request) eq(b CPartnerApps_FinishUploadDepot_Request) bool {
	return true && a.upload_token == b.upload_token && a.routing_id == b.routing_id &&
		a.app_id == b.app_id && a.depot_id == b.depot_id && a.build_flags == b.build_flags
}
...
pub fn cpartnerapps_getdepotbuildresult_response_unpack(buf []byte) ?CPartnerApps_GetDepotBuildResult_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			2 {
				i, res.error_msg = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn cpartnerapps_existingdrmupload_unpack(buf []byte) ?CPartnerApps_ExistingDRMUpload {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			...
			3 {
				i, res.actor_id = vproto.unpack_int32_field(cur_buf, tag_wiretype.wire_type) ?
			}
			5 {
				i, res.supplied_name = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			6 {
				i, res.flags = vproto.unpack_uint32_field(cur_buf, tag_wiretype.wire_type) ?
			}
			7 {
				i, res.mod_type = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			8 {
				i, res.timestamp = vproto.unpack_32bit_field(cur_buf, tag_wiretype.wire_type) ?
			}
			9 {
				i, res.orig_file_id = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			else {
				ii, v := vproto.unpack_unknown_field(cur_buf, tag_wiretype.wire_type)
				res.unknown_fields <<
...
pub fn (a CPartnerApps_ExistingDRMUpload) eq(b CPartnerApps_ExistingDRMUpload) bool {
	return true && a.file_id == b.file_id && a.app_id == b.app_id && a.actor_id == b.actor_id &&
		a.supplied_name == b.supplied_name && a.flags == b.flags && a.mod_type == b.mod_type &&
...
pub fn cpartnerapps_download_response_unpack(buf []byte) ?CPartnerApps_Download_Response {
	...
	for total < buf.len {
		...
		match tag_wiretype.tag {
			1 {
				i, res.download_url = vproto.unpack_string_field(cur_buf, tag_wiretype.wire_type) ?
			}
			2 {
				i, res.app_id = vproto.unpack_int32_field(cur_buf, tag_wiretype.wire_type) ?
			}
